class MessagesController < ApplicationController
    ...
    def create
        ...
        input_message_count = get_next_message_count
        obj = [
            @application.id, 
            @chat.number,
            params[:body],
    ...
    def get_next_message_count
        $redis_lock.lock("#{params[:application_id]}_#{params[:chat_id]}_message_counter", 2000) do |locked|
          counter = $redis.get("#{params[:application_id]}_#{params[:chat_id]}_message_counter")
          $redis.set("#{params[:application_id]}_#{params[:chat_id]}_message_counter", counter.to_i + 1)
          return counter
        end
      end
