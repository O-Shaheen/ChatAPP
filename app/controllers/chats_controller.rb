class ChatsController < ApplicationController
    ...
    def create
      chat_counter = get_next_chat_count
    ...
    private
    def get_next_chat_count
      $redis_lock.lock("#{params[:application_id]}_chat_counter", 2000) do |locked|
        counter = $redis.get("#{params[:application_id]}_chat_counter")
        $redis.set("#{params[:application_id]}_chat_counter", counter.to_i + 1)
        return counter
      end
    end
